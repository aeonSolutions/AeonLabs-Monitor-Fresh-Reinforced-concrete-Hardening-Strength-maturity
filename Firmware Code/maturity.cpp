...
void MATURITY_CLASS::init(INTERFACE_CLASS* interface, ONBOARD_SENSORS* onBoardSensors, mSerial* mserial, M_WIFI_CLASS* mWifi){
...
double MATURITY_CLASS::custom_strenght(int pos){
 ...
 void MATURITY_CLASS::request_measurment_data(){
    ...
    this->interface->setMCUclockFrequency( interface->SAMPLING_FREQUENCY);
...
bool MATURITY_CLASS::reinitialize_dataset_file(fs::FS &fs){
    ...
    if (datasetFile){
        ...
        this->interface->onBoardLED->statusLED( (uint8_t*)(const uint8_t[]){this->interface->onBoardLED->LED_RED}, 100,5); 
        return false;
    }
}
...
bool MATURITY_CLASS::save_measurment_record(fs::FS &fs){    
    ...
    if (datasetFile){      
        ...
        dataStr += String(roundFloat(this->onBoardSensors->onboardTHsensor->measurement[1], 2)) + ";" + String(roundFloat(this->onBoardSensors->onboardTHsensor->measurement[0],2)) + ";";  
        dataStr += String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[6], 2)) + ";";
        dataStr += String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[0], 3)) + ";" + String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[1], 3)) + ";";
        dataStr += String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[2], 3)) + ";" + String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[3], 3)) + ";";
        dataStr += String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[4], 3)) + ";" + String(roundFloat(this->onBoardSensors->onboardMotionSensor->measurement[5], 3))  + ";";
        ...
        dataStr += this->mWifi->InternetIPaddress + ";";
        dataStr += this->mWifi->requestGeoLocationDateTime + ";";
...
bool MATURITY_CLASS::gbrl_commands(String $BLE_CMD, uint8_t sendTo){
    ...
    if ($BLE_CMD == "$ufid"){
        ...
        this->interface->setMCUclockFrequency( interface->MAX_FREQUENCY);
        ...
        this->interface->setMCUclockFrequency( interface->CURRENT_CLOCK_FREQUENCY);
        this->interface->sendBLEstring( dataStr, sendTo);
        return true;
    }
    if( $BLE_CMD == "$pt"){
